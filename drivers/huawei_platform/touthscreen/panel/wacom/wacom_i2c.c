...
extern struct wacom_i2c_pen *g_pen_data;
...
extern int mipi_liszt_panel_enable_tp_sync(void);
...
static int wacom_chip_get_info(struct ts_chip_info_param *info)
{
	...
	wacom_query_device(g_ts_data.client, wac_data->features);
...
static int wacom_suspend(void)
{
	...
	if(wacom_power_hold == 1){
		TS_LOG_INFO("wacom need keep power on\n");
		return NO_ERR;
	}
	wacom_power_off();
	wacom_power_hold = 0;
...
{
	if (wacom_power_hold == 1) {
		if (regulator_is_enabled(wac_data->tp_vci) == 0) {
			wacom_power_on();
			wacom_gpio_reset();
			msleep(30);
		}
		TS_LOG_INFO("wacom already power on\n");
		return NO_ERR;
	}
	wacom_power_on();
	wacom_gpio_reset();
...
out:
        mipi_liszt_panel_enable_tp_sync();
...
static ssize_t wacom_get_recovery_flag_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int recovery_flag;
	recovery_flag = get_boot_into_recovery_flag();

	TS_LOG_INFO("touch_recovery_flag is %d\n", recovery_flag);
...
static DEVICE_ATTR(get_recovery_flag, S_IRUSR | S_IRGRP | S_IROTH, wacom_get_recovery_flag_show, NULL);
...
static struct attribute *wacom_attributes[] = {
	...
	&dev_attr_gpio_reset.attr,
      ...
      &dev_attr_get_recovery_flag.attr,
	NULL
};
...
static void set_touch_coord(struct wacom_i2c *wac_i2c, u8 *data, struct ts_cmd_node *out_cmd)
{
	...
	for (i = 0; i < FINGERNUM_IN_PACKET; i++) {
		if (*finger_num > 0) {
			...
			TS_LOG_DEBUG("report finger [%d] coord: x= %d, y= %d, tsw=%d\n", *finger_num, x, y, tsw);
			if (tsw){
				if (data[4] == 1) {
					if (x_pre == x && y_pre == y) {
						break;
			...
			if (!tsw) {
				tsw_num--;
				TS_LOG_DEBUG("now %d fingers touch in TP\n", tsw_num);
			}
			if(!tsw && tsw_num < 1){ //BTN_UP
				x_pre = 0;
				y_pre = 0;
				tsw_num = 0;
...
static int wacom_irq_bottom_half(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	if (data[2] ==  0x06)
		...
		wacom_report_pen_data(g_pen_data,wac_data,data);
	else if (data[2] ==  0x0c) 
