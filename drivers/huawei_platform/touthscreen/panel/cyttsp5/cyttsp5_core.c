...
static void cyttsp5_watchdog_work(struct work_struct *work)
{
    ...
    if(cd->cpdata->product_name && !strncmp(cd->cpdata->product_name, "liszt", strlen("liszt"))) {
        ignore_i2c_err = true;
    }
    rc = cyttsp5_hid_output_null_(cd);
    if(cd->cpdata->product_name && !strncmp(cd->cpdata->product_name, "liszt", strlen("liszt"))) {
        ignore_i2c_err = false;
    }
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if(get_info_flag){
        ...
        if (cd->cpdata->product_name && (strncasecmp(cd->cpdata->product_name, "liszt", strlen("liszt")) == 0 ||
			    strncasecmp(cd->cpdata->product_name, "mozart", strlen("mozart")) == 0)) {
            len += snprintf(buf + len, PAGE_SIZE,"Chip_Name:%s,",cd->cpdata->chip_name);
            length = sizeof(hw_tp_info)/sizeof(struct cyttsp5_tp_info);
            for(index = 0; index < length; index++) {
                retval = strncasecmp(hw_tp_info[index].product_name, cd->cpdata->product_name, strlen(hw_tp_info[index].product_name));
                if(retval == 0 && hw_tp_info[index].panel_id == cd->panel_id){
                    len += snprintf(buf + len, PAGE_SIZE, "Sensor_Id:%s,",hw_tp_info[index].module_name);
                    break;
                }
                if(index == (length - 1)){
                    len += snprintf(buf + len, PAGE_SIZE, "Sensor_Id:%s,", "null");
                }
            }
        } else {
...
static void cyttsp5_regulator_set(struct cyttsp5_power_control *power_ctrl)
{
    int rc = 0;
    if(power_ctrl->vci_power_type == VCI_LDO_CTRL_MODE){
        rc = regulator_set_voltage(power_ctrl->vci, 3300000, 3300000);
        if(rc < 0){
        ...
        u16 irq, size_t xfer_buf_size)
{
    ...
    TS_LOG_INFO("%s: ignore_i2c_err = %d.\n", __func__, ignore_i2c_err);
