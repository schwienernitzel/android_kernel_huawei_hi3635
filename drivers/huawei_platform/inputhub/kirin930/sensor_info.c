...
#define TEL_HUAWEI_NV_THEMECOLOR_NAME                    "THEMECOLOR"
#define TEL_HUAWEI_NV_THEMECOLOR_NUMBER                  16
#define TEL_HUAWEI_NV_THEMECOLOR_LENGTH                  16
#define TP_WHITHE "white"
#define TP_BLACK "black"
static struct work_struct als_tpcolor_work;
static char tp_color_data[TEL_HUAWEI_NV_THEMECOLOR_LENGTH] = "";
static char tp_color_result[TEL_HUAWEI_NV_THEMECOLOR_LENGTH] = "block";
#endif
...
#ifdef CONFIG_ADAPT_TP_TYPE
static int write_tpcolor_nv(char * tp_color)
{
	int ret = 0;
	write_info_t pkg_ap;
	int i,j;
	s16 temp_1[SENSOR_PLATFORM_EXTEND_DATA_SIZE/2]={0};
	uint8_t als_extend_data[SENSOR_PLATFORM_EXTEND_DATA_SIZE]={0};
	struct hisi_nve_info_user user_info;
	char *als_dts = NULL;
	hwlog_info("%s:%s", __func__,tp_color);

	memset(&user_info, 0, sizeof(user_info));
	user_info.nv_operation = NV_WRITE_TAG;
	user_info.nv_number = TEL_HUAWEI_NV_THEMECOLOR_NUMBER;
	user_info.valid_size = TEL_HUAWEI_NV_THEMECOLOR_LENGTH;
	strncpy(user_info.nv_name, TEL_HUAWEI_NV_THEMECOLOR_NAME, sizeof(user_info.nv_name));
	user_info.nv_name[sizeof(user_info.nv_name) - 1] = '\0';
	memcpy(user_info.nv_data, tp_color, TEL_HUAWEI_NV_THEMECOLOR_LENGTH);
	if ((ret = hisi_nve_direct_access(&user_info)) != 0) {
		hwlog_err("nve_direct_access write error(%d)\n", ret);
		return -1;
	}

	ret = of_property_read_string(als_dn, "compatible", (const char **)&als_dts);
	if(ret){
		hwlog_err("%s:read als chip info fail\n", __func__);
		return ret;
		}

	if((!strncmp(als_dts, "huawei,rohm_bh1745", sizeof("huawei,rohm_bh1745"))) || (!strncmp(als_dts, "huawei,avago_apds9251", sizeof("huawei,avago_apds9251"))))
	{
		if(strstr(tp_color,TP_WHITHE))
		{
			ret = fill_extend_data_in_dts(als_dn, "als_extend_data_1", als_extend_data, SENSOR_PLATFORM_EXTEND_DATA_SIZE, EXTEND_DATA_TYPE_IN_DTS_HALF_WORD);
		}
		else if(strstr(tp_color,TP_BLACK))
		{
			ret = fill_extend_data_in_dts(als_dn, "als_extend_data_2", als_extend_data, SENSOR_PLATFORM_EXTEND_DATA_SIZE, EXTEND_DATA_TYPE_IN_DTS_HALF_WORD);
		}
		if(ret)
		{
			hwlog_err("als_extend_dat:fill_extend_data_in_dts failed!\n");
			return ret;
		}
		...
		hwlog_info("temp_1[0]=%d,temp_1[10]=%d,temp_1[16]=%d\n",temp_1[0],temp_1[10],temp_1[16]);
...
int read_tpcolor_nv()
{
	char nv_tp_color[TEL_HUAWEI_NV_THEMECOLOR_LENGTH] = "";
	//static int tpcolor_nv_flag = 0;
	char *als_dts = NULL;
	int ret = 0;
	write_info_t pkg_ap;
	...
	memcpy(nv_tp_color, user_info.nv_data, TEL_HUAWEI_NV_THEMECOLOR_LENGTH);
	hwlog_err("%s:%s\n",__func__,nv_tp_color);
	ret = of_property_read_string(als_dn, "compatible", (const char **)&als_dts);
	if(ret){
		hwlog_err("%s:read als chip info fail\n", __func__);
		return ret;
	...
	if((!strncmp(als_dts, "huawei,rohm_bh1745", sizeof("huawei,rohm_bh1745"))) || (!strncmp(als_dts, "huawei,avago_apds9251", sizeof("huawei,avago_apds9251"))))
	{
		if(strstr(nv_tp_color,TP_WHITHE))
		{
			ret = fill_extend_data_in_dts(als_dn, "als_extend_data_1", als_extend_data, SENSOR_PLATFORM_EXTEND_DATA_SIZE, EXTEND_DATA_TYPE_IN_DTS_HALF_WORD);
		}
		else if(strstr(nv_tp_color,TP_BLACK))
		{
			ret = fill_extend_data_in_dts(als_dn, "als_extend_data_2", als_extend_data, SENSOR_PLATFORM_EXTEND_DATA_SIZE, EXTEND_DATA_TYPE_IN_DTS_HALF_WORD);
		}else{
			ret = fill_extend_data_in_dts(als_dn, "als_extend_data_1", als_extend_data, SENSOR_PLATFORM_EXTEND_DATA_SIZE, EXTEND_DATA_TYPE_IN_DTS_HALF_WORD);
...
int read_tpcolor_nv_recovery()
{
	...
	if((!strncmp(als_dts, "huawei,rohm_bh1745", sizeof("huawei,rohm_bh1745"))) || (!strncmp(als_dts, "huawei,avago_apds9251", sizeof("huawei,avago_apds9251"))))
	{
		...
		pkg_ap.wr_buf = als_extend_data;
		pkg_ap.wr_len = SENSOR_PLATFORM_EXTEND_DATA_SIZE;
		write_customize_cmd_noresp(pkg_ap.tag, CMD_ALS_SET_TPTYPE_REQ,
			als_extend_data, SENSOR_PLATFORM_EXTEND_DATA_SIZE);
		hwlog_info("%s-\n", __func__);
	}
	return 0;
}

static void als_tpcolor_handle(struct work_struct *work)
{
	char data_write[20] = "";
	if(strstr(tp_color_data, TP_BLACK))
	{
		strncpy(data_write, TP_BLACK, TEL_HUAWEI_NV_THEMECOLOR_LENGTH);
	}
	else if(strstr(tp_color_data, TP_WHITHE))
	{
		strncpy(data_write, TP_WHITHE, TEL_HUAWEI_NV_THEMECOLOR_LENGTH);
	}
	else
	{
		strncpy(tp_color_result, "fail", TEL_HUAWEI_NV_THEMECOLOR_LENGTH);
		hwlog_err("als_tpcolor_handle:input a invalid data\n");
		return;
	}

	int ret = write_tpcolor_nv(tp_color_data);
	if(ret != 0)
	{
		hwlog_err("tp_color_result:fail\n");
		strncpy(tp_color_result, "fail", TEL_HUAWEI_NV_THEMECOLOR_LENGTH);
	}
	else
	{
		hwlog_err("tp_color_result:success\n");
		strncpy(tp_color_result, "success", TEL_HUAWEI_NV_THEMECOLOR_LENGTH);
	}
	return;
}

static ssize_t attr_als_tpcolor_write(struct device *dev,
				struct device_attribute *attr, const char *buf, size_t count)
{
	if(NULL == buf)
		return 0;
	if(strlen(buf) > 10)
		return 0;
	hwlog_err("attr_als_tpcolor_write:%s\n", buf);
	strncpy(tp_color_data, buf, TEL_HUAWEI_NV_THEMECOLOR_LENGTH);
	strncpy(tp_color_result, "block", TEL_HUAWEI_NV_THEMECOLOR_LENGTH);
	schedule_work(&als_tpcolor_work);
	return count;
}

static ssize_t attr_als_tpcolor_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s", tp_color_result);
}

static DEVICE_ATTR(als_tpcolor, 0664, attr_als_tpcolor_show , attr_als_tpcolor_write);
#endif
...
#ifdef CONFIG_ADAPT_TP_TYPE
	INIT_WORK(&als_tpcolor_work, als_tpcolor_handle);
#endif
	hwlog_info("[%s] --", __func__);
