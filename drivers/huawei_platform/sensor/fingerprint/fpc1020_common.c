...
static int fpc1020_nav_write_sensor_1150a_setup(fpc1020_data_t *fpc1020);
/* -------------------------------------------------------------------- */
...
int fpc1020_setup_defaults(fpc1020_data_t *fpc1020)
{
    ...
    error = (fpc1020->chip.type == FPC1020_CHIP_1020A) ||
        ...
        (fpc1020->chip.type == FPC1020_CHIP_1150F) ? 0 : -EINVAL;
    ...
    if (fpc1020->chip.type == FPC1020_CHIP_1020A) {
        ...
        ptr = (fpc1020->chip.revision == 1) ? &fpc1020_setup_default_1150_a1 :
            NULL;
    } else if (fpc1020->chip.type == FPC1020_CHIP_1150F){
        ptr = (fpc1020->chip.revision == 1) ? &fpc1020_setup_default_1150_a1 :
            NULL;
    } else {
        ptr = NULL;
...
{
    int error = 0, error_acess = 0;
    int counter = FPC1020_RESET_RETRIES;
    int irq_counter = FPC1020_IRQ_WAIT_COUNT;
    u16 hardware_id = 0, temp_u16_gain = 0;
    u32 temp_u32_cntr = 0;
    fpc1020_reg_access_t reg;

    // 1. to conform spi interface is ok
    FPC1020_MK_REG_READ(reg, FPC102X_REG_HWID, &hardware_id);
    error_acess = fpc1020_reg_access(fpc1020, &reg);
    if (error_acess){
        dev_err(&fpc1020->spi->dev,"%s spi access HWID erro", __func__);
    } else {
        dev_dbg(&fpc1020->spi->dev,"Hardware id is 0x%x", hardware_id);
    }
    ...
    while (counter) {
        ...
        // 3. get irq gpio value
        irq_counter = FPC1020_IRQ_WAIT_COUNT;
        while(irq_counter) {
            irq_counter --;
            error = gpio_get_value(fpc1020->irq_gpio) ? 0 : -EIO;
            if (!error) {
                // wait for register changed
                udelay(500);
                break;
            } else {
                udelay(100);
        ...
        if(0 == irq_counter) {// no err print log may delay
            printk(KERN_INFO "%s irq_gpio value fail irq_counter = %d ...\n",
        ...
        if (!error) {
            ...
            // 4. if erro check spi interface
            printk(KERN_INFO "%s timed out,retrying ...\n", __func__);
            temp_u16_gain = 0xFF; //FPC102X_REG_ADC_SHIFT_GAIN reset value is 0
            FPC1020_MK_REG_READ(reg, FPC102X_REG_ADC_SHIFT_GAIN, &temp_u16_gain);
            error_acess = fpc1020_reg_access(fpc1020, &reg);
            if (error_acess) {
                dev_err(&fpc1020->spi->dev,"%s spi access GAIN erro", __func__);
            }

            temp_u32_cntr = 0x00; //FPC1150_REG_FNGR_DET_CNTR reset value is ff
            FPC1020_MK_REG_READ(reg,FPC1150_REG_FNGR_DET_CNTR, &temp_u32_cntr);
            error_acess = fpc1020_reg_access(fpc1020, &reg);
            if (error_acess) {
                dev_err(&fpc1020->spi->dev,"%s spi access CNTR erro", __func__);
            }
                dev_err(&fpc1020->spi->dev," reset GAIN = 0x%x, CNTR = 0x%x",
                    temp_u16_gain, temp_u32_cntr);
                udelay(1250);
        }
    }
    return error;
...
int fpc1020_nav_write_sensor_setup(fpc1020_data_t *fpc1020)
{
    switch (fpc1020->chip.type)
    {
        ...
        case FPC1020_CHIP_1150B:
        case FPC1020_CHIP_1150F:
        return fpc1020_nav_write_sensor_1150a_setup(fpc1020);
...
static int fpc1020_write_sensor_1150a_setup(fpc1020_data_t *fpc1020)
{
    ...
    temp_u8 = 0x34;//yongqiang 0x38
    FPC1020_MK_REG_WRITE(reg, FPC102X_REG_FINGER_DRIVE_DLY, &temp_u8);
    error = fpc1020_reg_access(fpc1020, &reg);
    if (error)
        goto out;

    temp_u64 = 0x333333333F3F3F3F;//0x373737373F3F3F3F; yongqiang
    FPC1020_MK_REG_WRITE(reg, FPC102X_REG_SAMPLE_PX_DLY, &temp_u64);
    error = fpc1020_reg_access(fpc1020, &reg);
    if (error)
        goto out;
...
int fpc1020_wait_finger_present(fpc1020_data_t *fpc1020)
{
    ...
    while (sleep_trys) {
        sleep_trys --;
        erro_sleep = fpc1020_sleep(fpc1020, false);
        if(0 > erro_sleep){ 
            // try to sleep sensor again
            fpc1020_wake_up(fpc1020);
            fpc1020_write_sensor_setup(fpc1020);
            error = fpc1020_dummy_capture(fpc1020);
            if (error < 0){
                dev_err(&fpc1020->spi->dev,"%s fpc1020_dummy_capture error =%d \n", __func__,error);
                return error;
                }
        } else {
            break;
    ...
    if (0 > erro_sleep) {
        // Warn but not return , and continue wait for finger irq
        dev_err(&fpc1020->spi->dev,"%s fpc1020 sleep error =%d, pm will be high, sleep_trys=%d\n", __func__, erro_sleep, sleep_trys);
    }
