...
static FPC1020_DEV_ATTR(diag, tap_enable ,    DEVFS_DIAG_MODE_RW);
...
static struct attribute *fpc1020_diag_attrs[] = {
    ...
    &fpc1020_attr_tap_enable.attr.attr,
...
int fpc1020_get_dts_data(struct device *dev, fpc1020_data_t *fpc1020_data)
{
    ...
    ret = of_property_read_u32(np, "spi-max-frequency",
        &fpc1020_data->spi_freq);
    if(ret)
    {
        fpc1020_data->spi_freq = FPC1020_SPI_CLOCK_SPEED;
        dev_err(dev,"failed to get spi freq from device tree\n");
    }
    dev_err(dev,"get spi freq (%d) from device tree\n", fpc1020_data->spi_freq);

#ifdef CONFIG_INPUT_FPC1020_NAV
    ret = of_property_read_u32_array(np, "fpc1020,nav_dir_key",
        fpc1020_data->nav.nav_dir_key, 4);
    if(ret)
    {
        fpc1020_data->nav.nav_dir_key[NAV_DIR_UP] = KEY_UP;
        fpc1020_data->nav.nav_dir_key[NAV_DIR_DOWN] = KEY_DOWN;
        fpc1020_data->nav.nav_dir_key[NAV_DIR_LEFT] = KEY_LEFT;
        fpc1020_data->nav.nav_dir_key[NAV_DIR_RIGHT] = KEY_RIGHT;
        dev_err(dev,"failed to get nav_dir_key config from device tree\n");
    }
    dev_err(dev,"get nav_dir_key config from device tree, [%d--%d--%d--%d]\n",
        fpc1020_data->nav.nav_dir_key[NAV_DIR_UP], fpc1020_data->nav.nav_dir_key[NAV_DIR_DOWN],
        fpc1020_data->nav.nav_dir_key[NAV_DIR_LEFT], fpc1020_data->nav.nav_dir_key[NAV_DIR_RIGHT]);

    ret = of_property_read_u32(np, "fpc1020,nav_dir_enable", &fpc1020_data->nav.nav_dir_enable);
    if(ret)
    {
        fpc1020_data->nav.nav_dir_enable = NAV_DIR_ENABLE_ALL;
        dev_err(dev,"failed to get nav_dir_enable from device tree, set enable all\n");
    }
    dev_info(dev,"nav_dir_enable value is %d\n", fpc1020_data->nav.nav_dir_enable);
...
switch (fpc1020->chip.type) {
    ...
    case FPC1020_CHIP_1150F :
        nav_para.nav_img_crop_x = FPC1150_NAV_IMG_CROP_X;
        nav_para.nav_img_crop_w = FPC1150_NAV_IMG_CROP_W;
        nav_para.nav_img_crop_y = FPC1150_NAV_IMG_CROP_Y;
        nav_para.nav_img_crop_h = FPC1150_NAV_IMG_CROP_H ;
        nav_para.nav_img_row_skip = FPC1150_NAV_IMG_ROW_SKIP;
        nav_para.nav_img_col_mask = FPC1150_NAV_IMG_COL_MASK;
        break;
    ...
    default:
        nav_para.nav_img_crop_x = FPC1020_NAV_IMG_CROP_X;
        nav_para.nav_img_crop_w = FPC1020_NAV_IMG_CROP_W;
        nav_para.nav_img_crop_y = FPC1020_NAV_IMG_CROP_Y;
        nav_para.nav_img_crop_h = FPC1020_NAV_IMG_CROP_H ;
        nav_para.nav_img_row_skip = FPC1020_NAV_IMG_ROW_SKIP;
        nav_para.nav_img_col_mask = FPC1020_NAV_IMG_COL_MASK;
        break;
    }
    nav_para.nav_img_w = NAV_IMG_W(nav_para.nav_img_crop_w, nav_para.nav_img_col_mask);
    nav_para.nav_img_h = NAV_IMG_H(nav_para.nav_img_crop_h, nav_para.nav_img_row_skip);
    nav_para.nav_img_size = NAV_IMG_SIZE(nav_para.nav_img_w, nav_para.nav_img_h);

    dev_err(&fpc1020->spi->dev, "w %d, h %d, size %d.\n", nav_para.nav_img_w, nav_para.nav_img_h, nav_para.nav_img_size);

    fpc1020->prev_img_buf = kzalloc(nav_para.nav_img_size * sizeof(u8), GFP_KERNEL);
    ...
    fpc1020->cur_img_buf = kzalloc(nav_para.nav_img_size * sizeof(u8), GFP_KERNEL);
...
static int fpc1020_release(struct inode *inode, struct file *file)
{
    ...
    if( NULL == fpc1020 ) {
        printk(KERN_ERR "%s fpc1020 is null!\n", __func__);
        return -1;
    }
...
static int fpc1020_spi_setup(fpc1020_data_t *fpc1020)
{
    ...
    fpc1020->spi->max_speed_hz = fpc1020->spi_freq;
                    ...
                    char *buf)
{
    ...
    else if(fpc_attr->offset == offsetof(fpc1020_diag_t, tap_enable)){
        val = (int)fpc1020->diag.tap_enable;
    }
                    ...
                    size_t count)
{
    ...
    if (fpc_attr->offset == offsetof(fpc1020_diag_t, spi_register)) {
        ...
        error = kstrtou64(buf, 0, &val);
        if (!error) {
            fpc1020->diag.tap_enable = val;
            printk("fpc1020 btap enable = %d \n",fpc1020->diag.tap_enable);
        }
    } else if(fpc_attr->offset == offsetof(fpc1020_diag_t, result)){
        error = kstrtou64(buf, 0, &val);
        if (!error)
...
int fpc1020_check_deadpixels_in_detect_zone(fpc1020_data_t *fpc1020, int index)
{
      ...
      int xp_1150[]  =   {16, 72, 128, 184},yp_1150[] =  { 8, 32, 64 };
      int* xp;
      int* yp;
    switch (fpc1020->chip.type) {
    case FPC1020_CHIP_1020A:
