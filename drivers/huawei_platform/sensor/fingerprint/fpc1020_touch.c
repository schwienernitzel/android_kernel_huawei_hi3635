...
static void dispatch_touch_event(fpc1020_data_t* fpc1020, int x, int y, int finger_status)
{
    ...
    switch (finger_status)
    {
        ...
        default:
            pr_info("[FPC/TOUCH]: undefined gesture events\n");
            break;
    }
}
...
int  fpc1020_input_init(fpc1020_data_t* fpc1020)
{
    ...
    if (!error)
    {
        if (nav_para.nav_img_size * 2 > fpc1020->huge_buffer_size)
        {
        ...
        else
        {
            ...
            p_curr_img = fpc1020->huge_buffer + nav_para.nav_img_size;
        }
    }
#endif
    if (!error)
...
{
    int sub;/*used by fpc1020_check_finger_present_sum*/
    u8 zones;
    u32 abs_x;
    u32 abs_y;
#ifdef DEBUG_TIME
           ...
           fpc1020->nav.enabled && (error >= 0))
    {
...
#else
        error = fpc1020_capture_buffer(fpc1020, p_prev_img, 0, nav_para.nav_img_size);
        ...
        memcpy(fpc1020->prev_img_buf,
               fpc1020->huge_buffer, nav_para.nav_img_size);
#endif
        ...
        while (!fpc1020->worker.stop_request && (error >= 0))
        {
            ...
            error = capture_nav_image(fpc1020);
#else
            error = fpc1020_capture_buffer(fpc1020, p_curr_img, 0, nav_para.nav_img_size);
#endif
#ifdef DEBUG_TIME
            dev_dbg(&fpc1020->spi->dev,
...
#endif // DEBUG_TIME
#ifdef FPC1020_NAV_HEAP_BUF
            fpc1021_get_movement(prevBuffer, curBuffer, &dx, &dy);
#else
            fpc1021_get_movement(p_prev_img, p_curr_img, &dx, &dy);
#endif
#ifdef DEBUG_TIME
            dev_dbg(&fpc1020->spi->dev,
