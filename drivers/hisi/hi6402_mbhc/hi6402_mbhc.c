...
static struct hi6402_mbhc_platform_data *g_pdata = NULL;
...
bool get_hi6402_support_exern_cable_status(struct hi6402_irq *p_irq)
{

	if(NULL == g_pdata){
		pr_err("%s : g_pdata is null\n", __FUNCTION__);
		return false;
	}

	if(g_pdata->hs_detect_extern_cable)
		return true;
	else
		return false;
}

EXPORT_SYMBOL(get_hi6402_support_exern_cable_status);

bool get_hi6402_headset_plug_in_status(struct hi6402_irq *p_irq)
{
	...
	if(check_headset_pluged_in(g_pdata))
		return true;
	else
		return false;
}

EXPORT_SYMBOL(get_hi6402_headset_plug_in_status);
...
void hi6402_plug_in_detect(struct hi6402_mbhc_platform_data *pdata)
{
	...
	if (pdata->hs_3_pole_max_voltage >= saradc_value) {
		...
		if(pdata->hs_detect_extern_cable &&
			(pdata->hs_extern_cable_min_voltage <= saradc_value &&
			 pdata->hs_extern_cable_max_voltage >= saradc_value)){
				pr_info("%s : set as extern_cable\n", __FUNCTION__);
				pdata->hs_status = HISI_JACK_EXTERN_CABLE;
...
static irqreturn_t hi6402_plugin_handler(int irq, void *data)
{
	...
	if(pdata->hs_detect_extern_cable && (HISI_JACK_HEADSET == pdata->hs_status))
		hi6402_irq_clr_comp2_irqs(pdata->p_irq);
...
static irqreturn_t hi6402_btnup_handler(int irq, void *data)
{
	...
	if (!check_headset_pluged_in(pdata))
		return IRQ_HANDLED;
	if(pdata->hs_detect_extern_cable){
		if(HISI_JACK_EXTERN_CABLE == pdata->hs_status){
			pr_info("%s(%u) : for extern cable return!\n", __FUNCTION__, __LINE__);
			return IRQ_HANDLED;
		}
	}
...
static irqreturn_t hi6402_btnup_comp2_cable_in_handler(int irq, void *data)
{
	struct hi6402_mbhc_platform_data *pdata =
			(struct hi6402_mbhc_platform_data *)data;
...
static irqreturn_t hi6402_btndown_comp2_cable_out_handler(int irq, void *data)
{
	struct hi6402_mbhc_platform_data *pdata =
			(struct hi6402_mbhc_platform_data *)data;
...
static int hi6402_mbhc_probe(struct platform_device *pdev)
{
	...
	pdata->hs_detect_extern_cable = false;
	...
	if (!match) {
		...
		if (!of_property_read_u32(node, "hisilicon,hs_extern_cable_min_voltage", &temp))
			pdata->hs_extern_cable_min_voltage = temp;
		else
			pdata->hs_extern_cable_min_voltage = 0;
		if (!of_property_read_u32(node, "hisilicon,hs_extern_cable_max_voltage", &temp))
			pdata->hs_extern_cable_max_voltage = temp;
		else
			pdata->hs_extern_cable_max_voltage = 0;
		if (!of_property_read_u32(node, "hs_detect_extern_cable", &temp)){
			pr_info("%s:get device info of hs_detect_extern_cable =0x%x\n",__FUNCTION__ ,temp);
			if(temp){
				pdata->hs_detect_extern_cable = true;
			} else {
				pdata->hs_detect_extern_cable = false;
			}
		} else {
	...
	if(pdata->hs_detect_extern_cable) {
		pdata->irq[HI6402_IRQ_BTNDOWN_COMP2] = platform_get_irq_byname(pdev, "btndown_comp2");
		if (0 > pdata->irq[HI6402_IRQ_BTNDOWN_COMP2]) {
			pr_err("get btndown_comp2 irq num error");
			return -ENOENT;
		}

		pdata->irq[HI6402_IRQ_BTNUP_COMP2] = platform_get_irq_byname(pdev, "btnup_comp2");
		if (0 > pdata->irq[HI6402_IRQ_BTNUP_COMP2]) {
			pr_err("get btnup_comp2 irq num error");
			return -ENOENT;
		}
	}
    ...
    if(pdata->hs_detect_extern_cable){
		/* irq request : btndown_comp2 */
		ret = devm_request_threaded_irq(dev, pdata->irq[HI6402_IRQ_BTNDOWN_COMP2], NULL,
						hi6402_btndown_comp2_cable_out_handler,
						IRQF_NO_SUSPEND | IRQF_ONESHOT,
						"btndown_comp2", pdata);
		if (0 > ret) {
			pr_err("request irq for btndown comp2 err\n");
			goto btndown_comp2_err;
		}

		/* irq request : btnup_comp2 */
		ret = devm_request_threaded_irq(dev, pdata->irq[HI6402_IRQ_BTNUP_COMP2], NULL,
						hi6402_btnup_comp2_cable_in_handler,
						IRQF_NO_SUSPEND | IRQF_ONESHOT,
						"btnup_comp2", pdata);
		if (0 > ret) {
			pr_err("request irq for btnup comp2 err\n");
			goto btnup_comp2_err;
		}
    }
	...
	if(pdata->hs_detect_extern_cable){
		hi6402_irq_write(pdata->p_irq, HI6402_MBHC_VREF_REG, 0x8A);
	}else{
...
btnup_comp2_err:
    free_irq(pdata->irq[HI6402_IRQ_BTNDOWN_COMP2], pdata);
btndown_comp2_err:
    free_irq(pdata->irq[HI6402_IRQ_BTNUP_COMP1], pdata);
btnup_comp1_err:
