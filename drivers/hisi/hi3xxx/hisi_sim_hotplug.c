...
#define STR_SIM_CARD_IN             "SIM_STATE = IN"
#define STR_CLEAR_SIM_CARD_STATE    "SIM_STATE = OUT"
...
static DEVICE_ATTR(state, S_IRUGO, sim0_hotplug_state_show, sim0_hotplug_state_store);
...
static int sys_add_sim_node(void)
{
	int retval;
	pr_info("%s, begin\n", __func__);
	struct device_attribute **attrs = sim_attributes;
	...
	sim_dev = device_create(sim_class, NULL, MKDEV(0, 0), NULL, "sim0");
	if(!sim_dev)
	{
		return -ENOMEM;
	}
	while ((attr = *attrs++))
	{
		retval = device_create_file(sim_dev, attr);
		if (retval)
		{
			device_destroy(sim_class, sim_dev->devt);
			return retval;
		}
	}
...
static void hisi_sim_det_msg_to_ccore(struct hisi_sim_hotplug_info *info, u8 sim_state)
{
	...
	if(sim_dev)
	{
		kobject_uevent_env(&(sim_dev->kobj), KOBJ_CHANGE, NULL);
		pr_info("%s: send kobj msg\n", __func__);
	}
...
{
	sim_class = class_create(THIS_MODULE, "sim");
	if (IS_ERR(sim_class))
	{
