...
static int __init hisi_cma_dev_init(void);
#else
static int __init hisi_cma_dev_init(void){}
#endif
...
static int __init cma_activate_area(struct cma *cma)
{
	...
	pr_err("cma->count %lu %lu\n",cma->count,pageblock_order);
...
int __init __dma_contiguous_reserve_area(phys_addr_t size, phys_addr_t base,
				       phys_addr_t limit, const char *name, struct cma **res_cma,int prot,int is_dynamic)
{
				 ...
				 int count)
{
	...
	if (pfn < cma->base_pfn || pfn >= cma->base_pfn + cma->count){
		pr_err("pfn:%x cma->base_pfn:%x cma->count:%x\n",pfn,cma->base_pfn,cma->count);
		return false;
	}
