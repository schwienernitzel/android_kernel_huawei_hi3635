...
void hi6402_irq_clr_comp2_irqs(struct hi6402_irq *irq)
{
	BUG_ON(NULL == irq);

	mutex_lock(&irq->irq_lock);
	hi6402_irq_write(irq, HI6402_REG_IRQ_0, 0x0C);
	mutex_unlock(&irq->irq_lock);
}
EXPORT_SYMBOL(hi6402_irq_clr_comp2_irqs);

static void hi6402_irq_eco_enable(struct hi6402_irq *irq, bool enable)
{
	if(enable)
	{
		if(get_hi6402_support_exern_cable_status() && get_hi6402_headset_plug_in_status()){
			/* eco off */
			...
			pr_info("%s : mbhc cmp on and eco disable for extern cable\n", __FUNCTION__);
		}else{
			/* mbhc cmp off */
...
void hi6402_irq_hs_micbias_enable(struct hi6402_irq *irq, bool enable)
{
	...
	if (enable) {
		...
		if(get_hi6402_support_exern_cable_status() && get_hi6402_headset_plug_in_status()){
			pr_info("%s : hs micbias not set disable for exern cable\n", __FUNCTION__);
		}else{
			/* hsmic pd */
