		...
		/* value */
			...
			if (copy_from_client(&operation->params[i].value.a,
				...
				break;
            		...
            		if(operation->params[i].value.a >= 0){
                        	unsigned int ion_shared_fd = operation->params[i].value.a;
                		drm_ion_handle = ion_import_dma_buf(drm_ion_client, ion_shared_fd);
                		if(IS_ERR(drm_ion_handle)){
                    			TCERR("in %s err: drm ion handle get failed!!! client=%p handle=%p fd=%d\n",__func__, drm_ion_client, drm_ion_handle, ion_shared_fd);
                    			ret = -EFAULT;
                    			break;
                		}
                		ret = ion_phys(drm_ion_client, drm_ion_handle, &drm_ion_phys, &drm_ion_size);
                        	if(ret){
                            		TCERR("in %s err: ion_phys get failed ret=%d!!! client=%p handle=%p fd=%d\n",__func__, ret, drm_ion_client, drm_ion_handle, ion_shared_fd);
                            		ret = -EFAULT;
                    			break;
				}
