...
#define DDR_DSS_PORT				((0x1 << 2)|(0x1 << 3)) 
#define DDR_VCODEC_PORT				((0x1 << 1))
#define SECOS_ACPU_PORT             ((0x1 << 7)|(0x1 << 10))
#define DDR_FN_SETSEC                 0xc6000001
#define DDR_FN_UNSETSEC               0xc6000002
				      ...
				      unsigned long align)
{
	...
	if (seccm_heap->heap_status == CREAT){
		...
		create_mapping_late(seccm_heap->base, 
				phys_to_virt(seccm_heap->base),
				seccm_heap->size,__pgprot(PROT_DEVICE_nGnRE));

		atfd_hisi_fn_smc(DDR_FN_SETSEC,seccm_heap->base,seccm_heap->size,seccm_heap->ddr_port);
		seccm_heap->heap_status = ALLOC;
	}

	unsigned long offset = gen_pool_alloc(seccm_heap->pool, size);

	mutex_unlock(&ion_seccm_mutex);
	if (!offset){
		pr_err("seccm_alloc size 0x%x avail size 0x%x\n",size,gen_pool_avail(seccm_heap->pool));
		       ...
		       unsigned long size)
{
	...
	if (gen_pool_avail(seccm_heap->pool) == seccm_heap->size){
		if (seccm_heap->heap_status == ALLOC){
			atfd_hisi_fn_smc(DDR_FN_UNSETSEC,seccm_heap->base,seccm_heap->size,seccm_heap->ddr_port);
			create_mapping_late(seccm_heap->base,
					phys_to_virt(seccm_heap->base),
					seccm_heap->size, __pgprot(PAGE_KERNEL));

			memset(phys_to_virt(seccm_heap->base),0x0,seccm_heap->size);
...
struct ion_heap *ion_seccm_heap_create(struct ion_platform_heap *heap_data)
{
	...
	seccm_heap->base = heap_data->base;
	...
	if (heap_data->id == ION_DRM_GRALLOC_HEAP_ID){
		seccm_heap->ddr_port = DDR_VCODEC_PORT|DDR_DSS_PORT|SECOS_ACPU_PORT;
	}else if(heap_data->id == ION_DRM_VCODEC_HEAP_ID){
		seccm_heap->ddr_port = DDR_VCODEC_PORT|SECOS_ACPU_PORT;
	}
