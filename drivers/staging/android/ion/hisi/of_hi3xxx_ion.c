...
static struct ion_platform_heap hisi_ion_heaps[] = {
	...
	/*
	{
		...
		.size = HISI_RESERVED_DRMGRALLOC_PHYMEM_SIZE/2,//64M
	},
	{
		.type = ION_HEAP_TYPE_SECCM,
		.id = ION_DRM_VCODEC_HEAP_ID,
		.name = "drm-vcodec-heap",
		.size = HISI_RESERVED_DRMVCODEC_PHYMEM_SIZE/2,//32M
	},
	*/
};
#endif
...
#ifdef CONFIG_HISI_CMA_RESERVE_MEMORY
	int dts_nheap = 0;
	struct ion_heap *dts_heap = NULL;
	struct device_node *node = pdev->dev.of_node;
	struct ion_platform_heap dts_heap_data;
	struct ion_dts_heap dts_part_data;
	err = of_property_read_u32(node, "ion-heap-n", &dts_nheap);
	if (err < 0){
		pr_err("in node %s please check dts-heap-n property of node %s heap numb %d\n",__func__, node->name,dts_nheap);
		goto nodtsheap;
	}

	pr_err("dts_nheap is %d\n",dts_nheap);
	for (i = 0; i < dts_nheap; i++) {
		unsigned int *pdata_u32 = &dts_part_data;

		memset(&dts_heap_data,0,sizeof(dts_heap_data));
		int j;
		for (j=0;j<5;j++){
			err = of_property_read_u32_index(node,"ion-heap-info",(i*5+j),pdata_u32);
			if (err < 0){
				pr_err("of_property_read_u32_planar_array %d\n",i);
				//goto out;
			}
			pdata_u32++;
		}

		dts_heap_data.type = dts_part_data.type;
		dts_heap_data.id   = dts_part_data.id;
		dts_heap_data.base = dts_part_data.base;
		dts_heap_data.align= dts_part_data.align;
		dts_heap_data.size = dts_part_data.size;
		dts_heap_data.priv = &pdev->dev;
		err = of_property_read_string_index(node,"ion-heap-name",i,&dts_heap_data.name);
		if (err < 0){
			pr_err("of_property_read_string_index failed\n");
			goto out;
		}

		pr_err("create heap %d "
			" ---> type: %d, id: %d, name: %s,"
			" base: 0x%lx, size: 0x%x\n", i,
			dts_heap_data.type, dts_heap_data.id, dts_heap_data.name,
			dts_heap_data.base, dts_heap_data.size);

		dts_heap = ion_heap_create(&dts_heap_data);
		if (IS_ERR_OR_NULL(dts_heap)) {
			dts_heap = 0;
			continue;
		} else {
			if (dts_heap_data.size)
				pr_info("ION heap %s created at %lx "
					"with size %x\n", dts_heap_data.name,
							  dts_heap_data.base,
							  dts_heap_data.size);
		...
		ion_device_add_heap(idev, dts_heap);
	}
