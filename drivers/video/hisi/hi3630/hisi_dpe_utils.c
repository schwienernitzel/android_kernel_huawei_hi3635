...
static unsigned  int g_gmp_State = 1;
...
void init_gmp(struct hisi_fb_data_type *hisifd)
{
	char __iomem *dss_base = 0;
	struct hisi_panel_info *pinfo = NULL;
	...
	if(pinfo->gmp_support != 1){
		HISI_FB_INFO("The panel do not need gmp_support.\n");
		return;
	}
	HISI_FB_INFO("start init gmp.\n");
	dss_base = hisifd->dss_base;
	...
	if (pinfo->gamma_support == 1) {
		//disable gamma
		set_reg(dss_base + DSS_DPP_GAMMA_OFFSET + GAMMA_BYPASS_EN, 0x1, 1, 0);
		set_reg(dss_base + DSS_DPP_GAMMA_OFFSET + GAMMA_CORE_GT, 0x2, 32, 0);//lut clock enable
		set_reg(dss_base + DSS_DPP_LCP_OFFSET + LCP_IGM_GT, 0x2, 32, 0);//xcc clock enable
		set_reg(dss_base + DSS_DPP_LCP_OFFSET + LCP_GMP_GT, 0x2, 32, 0);//gmp clock enable
		...
		if (pinfo->igm_lut_table_len > 0
			...
			for (i = 0; i < pinfo->igm_lut_table_len; i++) {
				set_reg(dss_base + DSS_DPP_LCP_OFFSET + (LCP_IGM_R_LUT + i * 4), pinfo->igm_lut_table_R[i], 32, 0);
				set_reg(dss_base + DSS_DPP_LCP_OFFSET + (LCP_IGM_G_LUT + i * 4), pinfo->igm_lut_table_G[i], 32, 0);
				set_reg(dss_base + DSS_DPP_LCP_OFFSET + (LCP_IGM_B_LUT + i * 4), pinfo->igm_lut_table_B[i], 32, 0);
			}
		}
		...
		if (pinfo->gamma_lut_table_len > 0
			...
			for (i = 0; i < pinfo->gamma_lut_table_len; i++) {
				set_reg(dss_base + DSS_DPP_GAMMA_OFFSET + (GAMMA_R_LUT + i * 4), pinfo->gamma_lut_table_R[i], 32, 0);
				set_reg(dss_base + DSS_DPP_GAMMA_OFFSET + (GAMMA_G_LUT + i * 4), pinfo->gamma_lut_table_G[i], 32, 0);
				set_reg(dss_base + DSS_DPP_GAMMA_OFFSET + (GAMMA_B_LUT + i * 4), pinfo->gamma_lut_table_B[i], 32, 0);				
			}
		}	
		//0:36bit input   1:24bit input (only IGM & GAMMA)
		set_reg(dss_base + DSS_DPP_GAMMA_OFFSET + GAMMA_SHORTBIT_EN, 0x1, 1, 0);
		//enable gamma
		set_reg(dss_base + DSS_DPP_GAMMA_OFFSET + GAMMA_BYPASS_EN, 0x0, 1, 0);
	}

    // GMP
	if (pinfo->gmp_support == 1) {
		//disable gmp
		set_reg(dss_base + DSS_DPP_LCP_OFFSET + LCP_GMP_BYPASS_EN, 0x1, 1, 0);   
		//gmp lut
		if (pinfo->gmp_lut_table_len > 0
			&& pinfo->gmp_lut_table_low32bit
			&& pinfo->gmp_lut_table_high4bit) {
			for (i = 0; i < pinfo->gmp_lut_table_len; i++) {
				pos1 = DSS_DPP_LCP_OFFSET + i * 0x800;                          
				for (j = 0; j < pinfo->gmp_lut_table_len; j++) {
					pos0 = pos1 + 0x80 * j;
					for (k = 0; k < pinfo->gmp_lut_table_len; k++) {
						set_reg(dss_base + pos0 +  k * 8,
							pinfo->gmp_lut_table_low32bit[(i * pinfo->gmp_lut_table_len + j) * pinfo->gmp_lut_table_len + k], 32, 0);
						set_reg(dss_base + pos0 +  k * 8 + 4,
							pinfo->gmp_lut_table_high4bit[(i * pinfo->gmp_lut_table_len + j) * pinfo->gmp_lut_table_len + k], 32, 0);
					}
				}
			}
		}
		//enable gmp
		set_reg(dss_base + DSS_DPP_LCP_OFFSET + LCP_GMP_BYPASS_EN, 0x0, 1, 0);	
		//0:36bit input (use GMP)   1:24bit input
		set_reg(dss_base + DSS_DPP_GAMMA_OFFSET + GAMMA_SHORTBIT_EN, 0x0, 1, 0);
	}
}
...
{
    g_gmp_State = value;
    HISI_FB_INFO(" g_gmp_State = %d" , g_gmp_State);       
}
...
{
	return snprintf(buf, PAGE_SIZE, "g_gmp_State = %d\n",g_gmp_State);
}