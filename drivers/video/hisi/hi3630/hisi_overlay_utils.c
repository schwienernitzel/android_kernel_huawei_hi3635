...
void hisi_drm_layer_config(struct hisi_fb_data_type *hisifd, dss_overlay_t *pov_req_prev, dss_overlay_t *pov_req)
{
	int i = 0;
	dss_layer_t *layer = NULL;
	struct hisifb_secure *secure_ctrl = NULL;

	BUG_ON((hisifd == NULL) || (pov_req == NULL) || (pov_req_prev == NULL));
	secure_ctrl = &(hisifd->secure_ctrl);
	BUG_ON(secure_ctrl == NULL);

	secure_ctrl->set_reg(HISI_DSS_BASE + DSS_GLB_PDP_SECU_CFG_OK, 0, 32, 0);

	for (i = 0; i < pov_req_prev->layer_nums; i++) {
		layer = &(pov_req_prev->layer_infos[i]);
		if (layer->src.secure_mode == 1) {
	...
	secure_ctrl->set_reg(HISI_DSS_BASE + DSS_GLB_PDP_SECU_CFG_OK, 1, 32, 0);
}
